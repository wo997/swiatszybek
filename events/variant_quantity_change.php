...
$variant_data = fetchRow("SELECT quantity, title, link, product_id, image, name FROM variant INNER JOIN products USING(product_id) WHERE variant_id = " . intval($input["variant_id"]));

$new_quantity = $variant_data["quantity"] + intval($input["quantity_difference"]);
if ($new_quantity < 0) $new_quantity = 0;

query("UPDATE variant SET quantity = $new_quantity WHERE variant_id = " . intval($input["variant_id"]));
...
if ($new_quantity > 0) // $variant_data["quantity"] == 0 && 
{
...
if ($variant_data["quantity"] > 0 && $new_quantity == 0)
{
