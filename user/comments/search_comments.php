<?php //->[search_comments]
  ...
  $canSee = "";
  if ($app["user"]["id"] && !$app["user"]["is_admin"]) $canSee = "AND (user_id = ".$app["user"]["id"]." OR accepted = 1)";
  ...
  echo getTableData([
    ...
    "renderers" => [
      "rating" => function($row) {return ratingBlock($row["rating"]);},
      "link" => function($row) {return getProductLink($row["product_id"],$row["link"]);},
