...
$rating_data = DB::fetchRow("SELECT AVG(rating) as avg, COUNT(rating) as count FROM comments
    WHERE product_id = " . $args["product_id"] . " AND accepted = 1 AND rating > 0");

DB::execute("UPDATE products SET cache_avg_rating = ?, cache_rating_count = ? WHERE product_id = " . $args["product_id"], [
