...
$status_id = intval($urlParts[3]);

$case_oplacono = $status_id == 1 ? ", oplacono = NOW()" : "";
$case_wyslano = $status_id == 2 ? ", wyslano = NOW()" : "";
$case_odebrano = $status_id == 3 ? ", odebrano = NOW()" : "";
...
query("UPDATE zamowienia SET status_id = ? $case_oplacono $case_wyslano $case_odebrano WHERE zamowienie_id = ?", [$status_id, $zamowienie_data["zamowienie_id"]]);
...
if ($status_id == 2) {
  ...
  $message .= getEmailFooter();
} else if ($status_id == 5) { // $zamowienie_data["dostawa"] == 'o'
...
$add_subtract_stock = "";
if ($status_id == 4 && $zamowienie_data["status_id"] != 4) {
  $add_subtract_stock = "+";
}
if ($status_id != 4 && $zamowienie_data["status_id"] == 4) {
  $add_subtract_stock = "-";
