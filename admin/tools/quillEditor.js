...
window.quillEditor = {
    myColorList:  ["rgb(255, 85, 118)", "rgb(255,43,0)", "#FFD700", "#6c1", "#3f00b5", "rgb(160,65,112)", "rgb(65,160,113)", "#e60000", "#ff9900", "#ffff00", "#008a00", "#0066cc", "#9933ff", "#ffffff", "#facccc", "#ffebcc", "#ffffcc", "#cce8cc", "#cce0f5", "#ebd6ff", "#bbbbbb", "#f06666", "#ffc266", "#ffff66", "#66b966", "#66a3e0", "#c285ff", "#888888", "#a10000", "#b26b00", "#b2b200", "#006100", "#0047b2", "#6b24b2", "#444444", "#5c0000", "#663d00", "#666600", "#003700", "#002966", "#3d1466", "#000000"],
    callback: null,
    ...
    open: (source,params = {}) => {
        ...
        var quillColorNode = elem("#quillEditor .background-color");
        if (params.colorNode) {
            quillColorNode.style.backgroundColor = params.colorNode.style.backgroundColor;
        ...
        var w = elem("#quillEditor .quill-editor-container");
        ...
        var qlContainer = elem(".quill-editor-container .ql-editor");
    ...
    save: () => {
        ...
        var content = elem("#quillEditor .ql-editor").innerHTML;
    ...
    hideCustomQuillButtons: () => {
        document.querySelectorAll(".quill-cloud").forEach(e=>{e.style.display = "";});
    },
    ...
    toggleQuillSize: (fromTarget = null) => {
        var e = elem("#quillEditor .quill-editor-container");
        if (fromTarget === null) {
        ...
        if (fromTarget && quillEditor.wrapper) {
            ...
            elem("#quillEditor .toggle_size").innerHTML = `<i class="fas fa-expand"></i>`;
        }
        else {
            e.style.width = "100%";
            elem("#quillEditor .toggle_size").innerHTML = `<i class="fas fa-compress"></i>`;
        }
    },
    isInTable: (elem) => {
        if (elem) {
    ...
    checkIfTable: () => {
        ...
        var table_menu = elem("#quillEditor .table_menu");
        var container = elem("#quillEditor .quill-wrapper");
        if (table) {
    ...
    markLastSelection: () => {
        ...
        if (nativeSelection && findParentByTagName(nativeSelection.anchorNode,"TD")) {
    ...
    toggleButtonsDisablationInTable: (disabled) => {
        document.querySelectorAll(".ql-header, .ql-list, .ql-video, .ql-table").forEach(e => {
    ...
    removeLink: () => {
        ...
        if (x && q && q.length == 0) quillEditor.selectText(quillEditor.active_elem);
        
        elem('#quillEditor .ql-myLink').click();
        quillEditor.hideCustomQuillButtons();
    },
    ...
    selectAnything: () => {
        var ed = elem("#quillEditor .quill-editor-container");
        var wasTop = ed.scrollTop;

        var selection = quillEditor.editor.getSelection();
        if (!selection) {
            quillEditor.editor.setSelection(quillEditor.lastSelection.index,quillEditor.lastSelection.length);
        }
    ...
    fixQLtooltip: () => {
        ...
        if (tool) {
            ...
            var maxleft = tool.parentNode.getBoundingClientRect().width - tool.getBoundingClientRect().width - 30;
            if (left > maxleft) tool.style.left = maxleft+"px";
            var maxtop = tool.parentNode.getBoundingClientRect().height - tool.getBoundingClientRect().height - 30;
    ...
    modifyNode: (prop, v) => {
        ...
        if (["alt","data-href"].indexOf(prop) != -1)
            ...
            quillEditor.active_elem.style[prop] = v;
        else if (prop == "youtube") {
            if (quillEditor.menu_sub == "myvideo") {
                quillEditor.active_elem.src = v;
            }
            else quillEditor.active_elem.src = getThumbnailFromYoutubeId(getIdFromYoutubeUrl(v));
        }
        else if (prop == "mylink") {
    ...
    fixHeight: (repeat = 0) => {
        var expectYmin = quillEditor.wrapper ? quillEditor.wrapper.getBoundingClientRect().height : 100000;
        var contentHeight = elem("#quillEditor .ql-editor").getBoundingClientRect().height;
        var h = Math.min(expectYmin,
        elem("#quillEditor .quill-wrapper").getBoundingClientRect().height
         - elem("#quillEditor .ql-toolbar.ql-snow").getBoundingClientRect().height - 10);
        
        elem("#quillEditor .quill-editor-container").style.minHeight = contentHeight < h ? h + "px" : "";
    ...
    loaded: () => {
        ...
        var toolbar = elem("#quillEditor .ql-toolbar.ql-snow");
        toolbar.parentNode.parentNode.appendChild(toolbar);

        elem('.quill-editor-container').insertAdjacentHTML("afterbegin",`<div class="background-color"></div>`);        
        ...
        elem("#quillEditor .ql-video").outerHTML += "";
        setTimeout(() => {
            elem("#quillEditor .ql-video").onclick = () => {
                //beforeImageInsert();
        ...
        elem("#quillEditor .ql-toolbar").insertAdjacentHTML("beforeend", `
        ...
        quillEditor.editor.on('text-change', (delta, oldDelta, source) => {
            ...
            for (a of delta.ops) {
                if (a.insert) {
                    ...
                    {
                        setTimeout(()=>{elem('#quillEditor .ql-myLink').click();},0);
                    }
                }
            }
            ...
            if (keyPressed && false) {
                ...
                if (ln && !isNodeLink) {
                    ...
                    quillEditor.editor.insertText(quillEditor.lastSelection.index-1," "+keyPressed);
        ...
        var observer = new MutationObserver((mutations) => {
            ...
            var x = elem("#quillEditor .ql-myLink:not([data-tooltip])");
            if (x) {
                x.innerHTML = '<i class="fas fa-link" style="transform: translateY(-2px);"></i>';
                x.setAttribute("data-tooltip","Ustaw zaznaczenie jako link");
            }
    
            var x = elem("#quillEditor .ql-shadow:not([data-tooltip])");
            if (x) {
                x.setAttribute("data-tooltip","Cień");
            }
    
            var x = elem("#quillEditor .ql-clean:not([data-tooltip])");
            ...
            document.querySelectorAll("#quillEditor break-line").forEach(e => {
                var n = e.nextSibling;
                e.classList.toggle("break-grow", n && n.tagName == "BREAK-LINE" || !n);
            });
        });

        observer.observe(elem('#quillEditor'), {
            childList: true,
            subtree: true
        });

        elem("#quillEditor .ql-editor").addEventListener("keydown", (e) => {
            quillEditor.hideCustomQuillButtons();
            if (e.key === "Escape") {
                elem("#quillEditor .ql-clean").click();
            }
            if (e.key === "ArrowRight") {
                var end = quillEditor.lastSelection.index + 1 == quillEditor.editor.getLength();
            ...
            var nativeSelection = getSelection();
            if (quillEditor.wasWasWasInTable || quillEditor.wasWasInTable || quillEditor.wasInTable) {// weird as hell    nativeSelection && findParentByTagName(nativeSelection.anchorNode,"TD") || quillEditor.wasInTable) {
            ...
            if (nativeSelection && findParentByTagName(nativeSelection.anchorNode,"TD")) {
                if (e.key === "Delete") {
                    ...
                    var deleted = false;
                    if (n && n.tagName == "BREAK-LINE" || (n && n.parentNode && n.parentNode.tagName == "BREAK-LINE")) {
        ...
        elem("#quillEditor").addEventListener("click", (event) => {
            removeClasses("ql-active-element");
            var touchQL = event.target == elem(".quill-editor-container");
            if (event.toElement == elem(".quill-wrapper") || touchQL) {
                /*if (touchQL) {
                ...
                elem("#quillEditor .ql-editor").classList.add("attention");
                setTimeout(()=>{
                    elem("#quillEditor .ql-editor").classList.remove("attention");
                },200);
            }
        });

        elem("#quillEditor .ql-editor").addEventListener("click", (e) => {
            ...
            if ((["IMG","VIDEO","A"].indexOf(quillEditor.active_elem.tagName) != -1) && !quillEditor.active_elem.classList.contains("ql-icon")) {
                var cloud = null;
                if (quillEditor.active_elem.classList.contains("ql-video")) {
                    if (quillEditor.active_elem.tagName == "VIDEO") {
                        ...
                        cloud = elem(".video-buttons");
                        document.getElementById("edit_yt_link").value = quillEditor.active_elem.src;
                        document.getElementById("szerokosc_yt").value = quillEditor.active_elem.style.width;
                    }
                    else {
                        quillEditor.menu_sub = 'youtube';
                        cloud = elem(".video-buttons");
                        document.getElementById("edit_yt_link").value = getUrlFromYoutubeId(getIdFromYoutubeThumbnail(quillEditor.active_elem.src));
                        document.getElementById("szerokosc_yt").value = quillEditor.active_elem.style.width;
                    }
                } else if (quillEditor.active_elem.tagName == "A") {
                    quillEditor.menu_sub = 'link';
                    cloud = elem(".my-link");
                    document.getElementById("my-link-href").value = quillEditor.active_elem.href;
                    document.getElementById("my-link-title").value = quillEditor.active_elem.innerHTML;
                    ...
                    cloud = elem(".image-buttons");
                    document.getElementById("szerokosc").value = quillEditor.active_elem.style.width;
                    document.getElementById("imgalt").value = quillEditor.active_elem.alt;
                    document.getElementById("img-href").value = quillEditor.active_elem.getAttribute("data-href");
                }
        ...
        /*var wasY = 0;
        elem("#quillEditor .quill-wrapper").addEventListener("scroll", function(e) {
            var nowY = e.srcElement.scrollTop;
            var diff = wasY - nowY;
            for (m of document.querySelectorAll("#quillEditor .quill-cloud, #quillEditor .table_menu")) {
            ...
            <div class="quill-wrapper stretch-vertical">
                ...
                <div class="image-buttons quill-cloud">
                    ...
                    <div style="margin-top:4px">
                        ...
                        <div class="btn primary" onclick="window.open(elem('#img-href').value);">Otwórz <i class="fas fa-external-link-alt"></i></div>
                    </div>
                </div>
                ...
                <div class="my-link quill-cloud" style="display: block; left: 296.258px; top: 139.219px;">
                    ...
                    <div style="text-align: center;padding-top: 5px;">
                        <div class="btn primary" onclick="window.open(elem('#my-link-href').value);">Otwórz <i class="fas fa-external-link-alt"></i></div>
            ...
            <div style="padding:10px;width:100%;max-width:300px;margin-top: -15px;">
                <label>
                    ...
                    <div style="display:flex">
                        ...
                        <button class="btn primary" onclick="quillEditor.insertVideo(elem('#videoUrl').value);hideParentModal(this);elem('#videoUrl').value=''">Wstaw</button>
                    </div>
                </label>
            </div>
        </div>
