...
// $message .= "Zobacz <a href='" . getZamowienieLink($zamowienie_link) . "' style='font-weight:bold;color:" . clr_primary . ";'>szczegóły zamówienia</a> oraz śledź jego status.";
...
if ($new_status_id == 2) {
    ...
    $message = getEmailHeader($zamowienie_data);
    $message .= "Uprzejmie informujemy, że zamówienie <a href='" . getZamowienieLink($zamowienie_data["link"]) . "' style='font-weight:bold;color:" . clr_primary . ";'>#" . $zamowienie_data["zamowienie_id"] . "</a> zostało spakowane oraz przekazane kurierowi.";
    if ($zamowienie_data["track"]) {
        $tracking_link = getTrackingLink($zamowienie_data["track"], $zamowienie_data["dostawa"], "");
        $message .= "<br><br>Możesz <b>śledzić przesyłkę</b> <a href='" . $tracking_link . "' style='font-weight:bold;color:" . clr_primary . ";'>tutaj</a>";
    }
    $message .= getEmailFooter();
    ...
    $message = getEmailHeader($zamowienie_data);
    $message .= "Uprzejmie informujemy, że odebrałaś/eś <a href='" . getZamowienieLink($zamowienie_data["link"]) . "' style='font-weight:bold;color:" . clr_primary . ";'>zamówienie #" . $zamowienie_data["zamowienie_id"] . "</a>.<br><br>";
    ...
    $message = getEmailHeader($zamowienie_data);
    $message .= "Uprzejmie informujemy, że anulowaliśmy <a href='" . getZamowienieLink($zamowienie_data["link"]) . "' style='font-weight:bold;color:" . clr_primary . ";'>zamówienie #" . $zamowienie_data["zamowienie_id"] . "</a>.<br><br>";
    ...
    $message .= "Zapłać teraz klikając w <a href='" . getZamowienieLink($zamowienie_data["link"]) . "' style='font-weight:bold;color:" . clr_primary . ";'>podany link</a>";
    $message .= getEmailFooter();
    ...
    $message = getEmailHeader($zamowienie_data);
    $message .= "Uprzejmie informujemy, że Twoje <a href='" . getZamowienieLink($zamowienie_data["link"]) . "' style='font-weight:bold;color:" . clr_primary . ";'>zamówienie #" . $zamowienie_data["zamowienie_id"] . "</a> oczekuje na odbiór <br>";
